# ちょこみんず
スースー不足のあなたへ
<br>チョコミント専門の記録&共有アプリ
<br>〜あなたにぴったりの「美味しい」と「爽快」を探そう！〜

サービスURL : https://chocomins.vercel.app/

# ■サービス概要
・レビューの記録
・メーカー・店舗の登録
・Xでシェア機能

# ■開発背景
私の身近にチョコミント好きの方がいます。
毎年、チョコミント商品が発売される時期になると、新作の食べ比べをしたり、東北の方までローカルなチョコミント商品を求めて足を運んでいます。
お互いに見つけたチョコミント商品を持ち寄って食べ比べや、店舗に食べに行ったこともありますが、万人受けの商品ではないため、まだまだ情報収集の場が少ないように思いました。

ふらっと出かけた先で偶然見つけたチョコミント商品。
こんなところにもあったのか！という情報をみんなで共有したい。
チョコミント好きのプラットフォームになればと、そんな思いで開発をしました。

# ■ターゲット層
20〜30代の層

# ■ターゲット層の理由
20~30代の層 チョコミントが好きな人が多い世代のため


# ■ユーザー獲得方法
Xでの共有機能、チョコミントやチョコが流行るイベントの時期にSNSを使って宣伝していきます。

# ■サービス利用のイメージ
チョコミントを食べた際に、食べたチョコレート登録し、レビューを記録します。
<br>「ちょこみんず」にいつ、どこで、何を食べたかを記録しておく事で、次回チョコミント商品を選ぶ際の参考になります。
店舗の場所やミント感も記録しておく事で、より効率的にチョコミント商品を探すことが出来ます。
<br>また、レビューは公開されるため他の人のレビューを閲覧することも可能です。
<br>チョコミント商品に迷ってもこの機能を使うことで効率的にチョコミント商品を見つけることが出来ます。




# ■主要な機能
- メーカー/店舗管理機能
- レビュー管理機能
- コメント機能


## レビュー記録管理

<table width="700" align="center" cellpadding="0" cellspacing="0" border="0">
  <tr>
    <th width="50%">レビュー記録一覧・詳細</th>
    <th width="50%">レビュー投稿</th>
  </tr>
  <tr>
    <td width="50%" align="center">
      <img src="https://i.gyazo.com/af4ce40e7f79c72427dc2f767c5d63fb.gif" width="350">
    </td>
    <td width="50%" align="center">
     <img src="https://i.gyazo.com/0c598e26f724270468509ab0a2d7ed16.gif" width="350">
    </td>
  </tr>
  <tr>
    <td width="50%">
      他のユーザーのレビュー記録を閲覧。レビュー記録の参考に出来ます※ログイン不要
    </td>
    <td width="50%">
      メーカー / 店舗を選択または、作成し、レビューを登録できます。レビュー内容は後から編集/削除可能
    </td>
  </tr>
</table>

<br>

<table width="700" align="center" cellpadding="0" cellspacing="0" border="0">
  <tr>
    <th width="50%">コメント</th>
    <th width="50%">住所オートコンプリート</th>
  </tr>
  <tr>
    <td width="50%" align="center">
      <img src="https://i.gyazo.com/a5881a27a2d8bcd8e1440ef8794c94af.gif" width="350">
    </td>
    <td width="50%" align="center">
      <img src="https://i.gyazo.com/bba3fbb1a09941e7a0740316719d8a1a.gif" width="350">
    </td>
  </tr>
  <tr>
    <td width="50%">
      チョコミントレビューにコメントで交流。チョコミント商品について盛り上がろう！
    </td>
    <td width="50%">
      チョコミントを買った場所、食べた場所を記録できます。場所記録時にはオートコンプリートで場所の検索候補を表示し検索をサポートします。
    </td>
  </tr>
</table>

<br>

## メーカー / 店舗管理、SNSシェア

<table width="700" align="center" cellpadding="0" cellspacing="0" border="0">
  <tr>
    <th width="50%">メーカー / 店舗の一覧</th>
    <th width="50%">SNSシェア</th>
  </tr>
  <tr>
    <td width="50%" align="center">
      <img src="https://i.gyazo.com/09d7675ecc8d08e9559a0e8cdbeaeef4.gif" width="350">
    </td>
    <td width="50%" align="center">
      <img src="https://i.gyazo.com/35744efd31c40dc19cf480b6fc2c5c27.gif" width="350">
    </td>
  </tr>
  <tr>
    <td width="50%">
      メーカーを一覧で管理。全ての人が共有で使用できます。
    </td>
    <td width="50%">
      おすすめのチョコミント商品をXでシェアして共有しよう！
    </td>
  </tr>
</table>

<br>

## ユーザー機能

<table width="700" align="center" cellpadding="0" cellspacing="0" border="0">
  <tr>
    <th width="50%">ユーザー登録・ログイン</th>
    <th width="50%">マイページ</th>
  </tr>
  <tr>
    <td width="50%" align="center">
      <img src="https://i.gyazo.com/06d49a772cf39d82a44a860ab8d16477.gif" width="350">
    </td>
    <td width="50%" align="center">
      <img src="https://i.gyazo.com/f2bad80660a00ea2d85e739b0d5e08a5.gif" width="350">
    </td>
  </tr>
  <tr>
    <td width="50%">
      メールアドレス＋パスワードでログイン。
    </td>
    <td width="50%">
      マイページではプロフィールの閲覧・編集ができます。
    </td>
  </tr>
</table>


# ■技術スタック
| カテゴリ | 技術 |
|---|---|
| バックエンド | TypeScript:5.9.3 / Next.js:15.4.10 |
| フロントエンド | TypeScript:5.9.3 / React:19.1.2 / Next.js:15.4.10 |
| CSSフレームワーク | Tailwind CSS + shadcn/ui |
| ORM | Prisma:6.19.0 |
| Web API |  Google Places API / Google Maps JavaScript API |
| データベースサーバー | PostgreSQL(Supabase) |
| ストレージ | Supabase Storage |
| 認証 | NextAuth.js |
| 環境構築 | Docker |
| CI/CD | GitHub Actions |
| アプリケーションサーバー | Vercel |
| テスト | jest:30.2.0 |



# ■ER図
[![Image from Gyazo](https://i.gyazo.com/a998630e71ed1e042e12392e235a3a3c.png)](https://gyazo.com/a998630e71ed1e042e12392e235a3a3c)
